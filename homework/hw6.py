...
variants = ['кот','шеф','мозг','лес','фолк','код','рот','мёд','лук','лес','год','час','друг','жена','муж','айфон','работа',]
user_input = int(input('Введите количество пословиц:'))
new_variants = [ ]

for i in range(user_input):
    while len(new_variants) < user_input:
        proverb = random.choice(proverbs)
        word = random.choice(variants)
        proverbs.remove(proverb)
        variants.remove(word)
        proverb_new = proverb.replace('Ум',word)
        new_variants.append(proverb_new)
    else:
        break
print(new_variants)
